...
update_normalization(struct scaling_factor sf,
  ...
  if (sf.multiply) {
    ...
    int extra_factor_exp2 =
        stdio_exp2(get_bit_access(extra_multiplicative_factor));
...
static struct double_components
get_normalized_components(bool_t negative,
                          printf_size_t precision,
  ...
  bool_t close_to_representation_extremum =
      ((-floored_exp10 + (int)precision) >= DBL_MAX_10_EXP - 1);
  if (close_to_representation_extremum) {
    // We can't have a normalization factor which also accounts for the precision,
    // i.e. moves some decimal digits into the mantissa, since it's
    // unrepresentable, or nearly unrepresentable. So, we'll give up early on
    // getting extra precision...
...
static void print_exponential_number(struct printf_output_gadget* output,
                                     double number,
                                     printf_size_t precision,
                                     printf_size_t width,
                                     printf_flags_t flags,
  ...
  // This number will decrease gradually (by factors of 10) as we "extract" the
  // exponent out of it
  ...
  // the decimal part after decimal exponent extraction, and the base-10 exponent
  // part. For both of these, the value of 0 has a special meaning, but not the
  ...
  if (flags & FLAGS_ADAPT_EXP) {
    ...
    // "precision" is the number of _significant digits_, and this is when we
    // "translate" the precision value to an actual number of decimal digits.
  ...
  bool_t should_skip_normalization =
  ...
  printf_size_t decimal_part_width =
      ((flags & FLAGS_LEFT) && exp10_part_width)
          ?
          // We're padding on the right, so the width constraint is the exponent
          // part's problem, not the decimal part's, so we'll use as many
          // characters as we need:
          0U
          :
          // We're padding on the left; so the width constraint is the decimal
          // part's problem. Well, can both the decimal part and the exponent part
  ...
  print_broken_up_decimal(decimal_part_components,
                          output,
                          precision,
                          decimal_part_width,
                          flags,
