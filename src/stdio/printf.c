...
static inline void format_string_loop(
struct printf_output_gadget* output, const char* format, va_list args)
{
  while (*format)
  {
    if (*format != '%') {
    ...
    switch (*format) {
      ...
      case 's' : {
        ...
        if (p == NULL) {
          ...
          printf_size_t l = stdio_strnlen(p, precision ? precision : PRINTF_MAX_POSSIBLE_BUFFER_SIZE);
          // pre padding
          if (flags & FLAGS_PRECISION) {
...
int stdio_vusfprintf(void (*out)(char c, void* extra_arg), void* extra_arg, const char* format, va_list arg)
{
...
int stdio_usfprintf(void (*out)(char c, void* extra_arg), void* extra_arg, const char* format, ...)
{
  va_list args;
  va_start(args, format);
