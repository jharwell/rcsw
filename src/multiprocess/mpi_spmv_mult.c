...
struct mpi_spmv_mult* mpi_spmv_mult_init(
  ...
  RCSW_FPC_NV(NULL, NULL != config);
  RCSW_ER_MODULE_INIT();

  ER_DEBUG("Initializing sparse matrix -> vector multiplier");

  /* All MPI processes perform the same basic initialization */
  struct mpi_spmv_mult* mult =
    rcsw_alloc(NULL, sizeof(struct mpi_spmv_mult), RCSW_NONE);

  RCSW_CHECK_PTR(mult);

  mult->matrix         = config->matrix;
  mult->mpi_rank       = config->mpi_rank;
  mult->mpi_world_size = config->mpi_world_size;
  mult->vector_in      = NULL;
  ...
  if (0 == mult->mpi_rank) {
    ...
    mult->rank_alloc_elts =
      rcsw_alloc(NULL, mult->mpi_world_size * sizeof(int), RCSW_ZALLOC);
    RCSW_CHECK_PTR(mult->rank_alloc_elts);

    mult->rank_alloc_rows =
      rcsw_alloc(NULL, mult->mpi_world_size * sizeof(int), RCSW_ZALLOC);
    ...
    mult->row_sizes =
      rcsw_alloc(NULL, mult->n_rows_init * sizeof(int), RCSW_ZALLOC);
  ...
  mult->row_owners =
    rcsw_alloc(NULL, (mult->n_rows_init + 2) * sizeof(int), RCSW_ZALLOC);
...
void mpi_spmv_mult_destroy(struct mpi_spmv_mult* mult) {
  if (mult) {
    if (mult->row_owners) {
      rcsw_free(mult->row_owners, RCSW_NONE);
    }
    if (mult->rank_alloc_elts && mult->mpi_rank == 0) {
      rcsw_free(mult->rank_alloc_elts, RCSW_NONE);
    }
    if (mult->rank_alloc_rows && mult->mpi_rank == 0) {
      rcsw_free(mult->rank_alloc_rows, RCSW_NONE);
    }
    if (mult->rank_alloc_row_prefix_sums && mult->mpi_rank == 0) {
      rcsw_free(mult->rank_alloc_row_prefix_sums, RCSW_NONE);
    }
    if (mult->row_sizes && mult->mpi_rank == 0) {
      rcsw_free(mult->row_sizes, RCSW_NONE);
    }
    if (mult->vector_in) {
...
status_t mpi_spmv_mult_ds_init(struct mpi_spmv_mult* const mult) {
  ...
  if (0 != mult->mpi_rank) {
    ...
    struct csmatrix_config matrix_config = {.n_rows    = mult->n_rows_alloc,
                                            .n_nz_elts = mult->n_elts_alloc,
                                            .n_cols    = mult->n_cols_init,
                                            .type      = ekCSMATRIX_DOUBLE,
                                            .flags     = 0};
    mult->matrix                         = csmatrix_init(NULL, &matrix_config);
    RCSW_CHECK_PTR(mult->matrix);
  }
  ...
  ER_TRACE("Rank%d: Initialize input/output vector (%d/%d elts)",
           ...
           mult->n_cols_init,
           mult->n_rows_alloc);
  struct darray_config vector_config = {.init_size = mult->n_cols_init,
  ...
  if (0 != mult->mpi_rank) {
    mult->vector_in         = darray_init(NULL, &vector_config);
    vector_config.init_size = mult->n_rows_alloc;
    vector_config.max_elts  = mult->n_rows_alloc;
  } else {
    vector_config.init_size = mult->n_rows_init;
    vector_config.max_elts  = mult->n_rows_init;
    mult->vector_in         = NULL;
  }
  mult->vector_out = darray_init(NULL, &vector_config);
  ...
  RCSW_CHECK(MPI_SUCCESS == MPI_Type_create_struct(count,
                                                   blocklens,
                                                   indices,
                                                   old_types,
                                                   &mult->spmv_comm_type));
...
status_t mpi_spmv_mult_distribute(struct mpi_spmv_mult* const mult,
   ...
   */
  RCSW_CHECK(
    OK == csmatrix_resize(mult->matrix, mult->n_rows_alloc, mult->n_elts_alloc));
...
struct darray* mpi_spmv_mult_exec(struct mpi_spmv_mult* const mult) {
   ...
   */
  RCSW_CHECK(0 == MPI_Gatherv(darray_data_get(mult->vector_out, 0),
                              mult->n_rows_alloc,
                              MPI_DOUBLE,
