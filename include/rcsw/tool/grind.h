 ...
 */
#define RCSW_GRIND_INTERVAL (1 << RCSW_MODFLAGS_SHIFT)
 ...
 *
 * If this flag is not passed, then stats for a grindee will never be reset, and
 * once full no more stats will be gathered.
 */
#define RCSW_GRIND_RESET_AUTO (1 << (RCSW_MODFLAGS_SHIFT + 1))
 ...
 */
#define RCSW_GRIND_REPORT_AUTO (1 << (RCSW_MODFLAGS_SHIFT + 2))
 ...
 */
#define RCSW_GRIND_REPORT_REQ_FULL (1 << (RCSW_MODFLAGS_SHIFT + 3))
 ...
 * Valid in any mode.
 */
#define RCSW_GRIND_REPORT_DATAPOINTS (1 << (RCSW_MODFLAGS_SHIFT + 4))
 ...
 * Valid in any mode.
 */
#define RCSW_GRIND_REPORT_HISTOGRAM (1 << (RCSW_MODFLAGS_SHIFT + 5))
...
struct grind_config {
  ...
  /**
   * Number of grindee names in \ref grind_config.names.
   */
  size_t n_inst;
