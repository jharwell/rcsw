...
static void arr32_permute_cb(void *) {
  ++arr32_permute_count;
}
...
CATCH_TEST_CASE("Bit Reverse Test", "[utils]") {
  ...
  CATCH_REQUIRE(RCSW_REVTBL8(foo8) == 0x80);
  CATCH_REQUIRE(RCSW_REVTBL16(foo16) == 0x8000);
  CATCH_REQUIRE(RCSW_REVTBL32(foo32) == 0x80000000);
  ...
  CATCH_REQUIRE(RCSW_REVTBL8(foo8) == 0x1);
  CATCH_REQUIRE(RCSW_REVTBL16(foo16) == 0x1);
  CATCH_REQUIRE(RCSW_REVTBL32(foo32) == 0x1);
};

CATCH_TEST_CASE("CRC Test", "[utils]") {
  uint8_t data[] = {0x4d, 0x54, 0x30, 0x30, 0x01, 0x02, 0x03, 0x04};
  uint32_t crc = utils_crc32_eth(data, 8);
  CATCH_REQUIRE(RCSW_BSWAP32(crc) == 0xb5e0332);

  crc = utils_crc32_ethl(data, 8);
...
CATCH_TEST_CASE("Bitmask Test", "[utils]") {
  ...
  CATCH_REQUIRE(RCSW_BITS_HI32(foo32) == 0x34560000);
  CATCH_REQUIRE(RCSW_BITS_LO32(foo32) == 0x7890);

  CATCH_REQUIRE(RCSW_BITS_HI64(foo64) == 0xFFAABBEE00000000);
  CATCH_REQUIRE(RCSW_BITS_LO64(foo64) == 0x00112233);
};
...
CATCH_TEST_CASE("mem_dump Test", "[utils]") {
  uint8_t buf[] = {
      0xF, 0xE,  0x17, 0x14, 0x3, 0x78, 0xFF, 0x46, 0xF, 0xE,  0x17, 0x14,
      0x3, 0x78, 0xFF, 0x46, 0xF, 0xE,  0x17, 0x14, 0x3, 0x78, 0xFF, 0x46,
      0xF, 0xE,  0x17, 0x14, 0x3, 0x78, 0xFF, 0x46, 0xF, 0xE,  0x17, 0x14,
      0x3, 0x78, 0xFF, 0x46, 0xF, 0xE,  0x17, 0x14, 0x3, 0x78, 0xFF, 0x46,
      0xF, 0xE,  0x17, 0x14, 0x3, 0x78, 0xFF, 0x46, 0xF, 0xE,  0x17, 0x14,
      0x3, 0x78, 0xFF, 0x46, 0xF, 0xE,  0x17, 0x14, 0x3, 0x78, 0xFF, 0x46,
  ...
  utils_mem_dump8((uint8_t *)buf, sizeof(buf));
  printf("\n");
  utils_mem_dump16((uint16_t *)buf, sizeof(buf) / 2);
  printf("\n");
  utils_mem_dump32((uint32_t *)buf, sizeof(buf) / 4);
  printf("\n");
  utils_mem_dump8v((uint8_t *)buf, sizeof(buf));
  printf("\n");
  utils_mem_dump16v((uint16_t *)buf, sizeof(buf) / 2);
  printf("\n");
  utils_mem_dump32v((uint32_t *)buf, sizeof(buf) / 4);
};

CATCH_TEST_CASE("utils_mem_copy Test", "[utils]") {
  uint8_t buf1[1000];
  uint8_t buf2[1000];
  ...
  for (i = 4; i < sizeof(buf1) / sizeof(uint32_t); i += 4) {
    ...
    utils_string_gen((char *)buf1, i);
    utils_mem_cpy32(buf2, buf1, i);
    CATCH_REQUIRE(memcmp(buf1, buf2, sizeof(buf2)) == 0);
  } /* for() */
};

CATCH_TEST_CASE("utils_mem_bswap Test", "[utils]") {
  ...
  CATCH_REQUIRE(OK == utils_mem_bswap16(in16, RCSW_ARRAY_ELTS(in16)));
  CATCH_REQUIRE(0 == memcmp(in16, out16, RCSW_ARRAY_ELTS(in16)));

  CATCH_REQUIRE(OK == utils_mem_bswap32(in32, RCSW_ARRAY_ELTS(in32)));
...
CATCH_TEST_CASE("time Test", "[utils]") {
  ...
  CATCH_REQUIRE(utils_ts_make_abs(&t1, &t2) == OK);
...
CATCH_TEST_CASE("checksum Test", "[utils]") {
   ...
   */
  CATCH_REQUIRE(utils_xchks8((uint8_t *)data.elts.data(), data.elts.size(), 0) >
                0);
  CATCH_REQUIRE(
      utils_xchks16((uint16_t *)data.elts.data(), data.elts.size(), 0) > 0);
  CATCH_REQUIRE(
      utils_xchks32((uint32_t *)data.elts.data(), data.elts.size(), 0) > 0);
  CATCH_REQUIRE(utils_achks8((uint8_t *)data.elts.data(), data.elts.size(), 0) >
                0);
  CATCH_REQUIRE(
      utils_achks16((uint16_t *)data.elts.data(), data.elts.size(), 0) > 0);
  CATCH_REQUIRE(
      utils_achks32((uint32_t *)data.elts.data(), data.elts.size(), 0) > 0);
  CATCH_REQUIRE(
      utils_crc32_brown((uint8_t *)data.elts.data(), 0, data.elts.size()) > 0);

  uint32_t hash;
  CATCH_REQUIRE(OK ==
                utils_hash_djb(data.elts.data(), data.elts.size(), &hash));
  CATCH_REQUIRE(hash > 0);
}

CATCH_TEST_CASE("misc Test", "[utils]") {
  CATCH_REQUIRE(utils_reflect32(0xF0021001, 32) == 0x8008400F);
  CATCH_REQUIRE(utils_reflect32(0xF0000001, 16) == 0x8000);
  ...
  utils_elt_swap(arr2, sizeof(uint32_t), 0, 1);
  utils_elt_swap(arr2, sizeof(uint32_t), 2, 3);
