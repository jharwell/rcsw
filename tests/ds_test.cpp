...
status_t ds_init(darray_params *const params) {
  ...
  params->elements = reinterpret_cast<dptr_t *>(
    malloc(darray_element_space(TH_NUM_ITEMS * 2, params->elt_size)));
...
status_t ds_init(multififo_params *const params) {
  RCSW_ER_MODULE_INIT();
  params->elements = reinterpret_cast<dptr_t *>(
    malloc(multififo_element_space(params->elt_size, TH_NUM_ITEMS)));
  params->meta = reinterpret_cast<dptr_t *>(
    malloc(multififo_meta_space(params->elt_size, TH_MULTIFIFO_CHILDREN)));
  return OK;
}
...
status_t ds_init(binheap_params *const params) {
  RCSW_ER_MODULE_INIT();
  params->elements = reinterpret_cast<dptr_t *>(
    malloc(binheap_element_space(TH_NUM_ITEMS, params->elt_size)));
  memset(params->elements,
         0,
...
status_t ds_init(adj_matrix_params *const params) {
  ...
  params->elements = reinterpret_cast<dptr_t *>(
    malloc(adj_matrix_element_space(TH_NUM_ITEMS, true)));
...
status_t ds_init(bstree_params *const params) {
  if (params->flags & RCSW_DS_BSTREE_OS) {
    ...
    params->elements = reinterpret_cast<dptr_t *>(
      malloc(ostree_element_space(TH_NUM_ITEMS, params->elt_size)));
  } else {
    params->meta =
      reinterpret_cast<dptr_t *>(malloc(bstree_meta_space(TH_NUM_ITEMS)));
    params->elements = reinterpret_cast<dptr_t *>(
      malloc(bstree_element_space(TH_NUM_ITEMS, params->elt_size)));
  }
  return OK;
}
status_t ds_init(matrix_params *const params) {
  RCSW_ER_MODULE_INIT();
  params->elements = reinterpret_cast<dptr_t *>(malloc(
    matrix_element_space(params->n_rows, params->n_cols, params->elt_size)));

  return OK;
}
RCSW_CONST status_t ds_init(dyn_matrix_params *const) { return OK; }
