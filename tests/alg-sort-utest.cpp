...
static void test_insertion_sort(size_t n_elts) {
  ...
  for (int i = 0; i < (int)n_elts - 1; ++i) {
    CATCH_REQUIRE(th::cmpe<T>(data.elts.data() + i, data.elts.data() + i + 1) <=
                  0);
  } /* for(i..) */
}
...
void test_qsort(size_t n_elts) {
  ...
  for (int i = 0; i < (int)n_elts - 1; ++i) {
    CATCH_REQUIRE(th::cmpe<T>(data1.elts.data() + i, data1.elts.data() + i + 1) <=
                  0);
    CATCH_REQUIRE(th::cmpe<T>(data2.elts.data() + i, data2.elts.data() + i + 1) <=
                  0);
  } /* for(i..) */
}
